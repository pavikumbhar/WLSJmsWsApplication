...
public abstract class QuartzScheduler {
	...
	protected SchedulerFactoryBean createScheduler(String schedulerName, Trigger trigger) {
		...
		/**
		 * Naturally, Quartz with the DB requires references to the data source and
		...
		/**
		 * This name is persisted as SCHED_NAME in db. for local testing could change to
		 * unique name to avoid collision with dev server
		 */
		quartzScheduler.setSchedulerName(schedulerName);

		/**
		 * Will update database cron triggers to what is in this jobs file on each
		 * deploy. Replaces all previous trigger and job data that was in the database.
		 * YMMV
		 */
