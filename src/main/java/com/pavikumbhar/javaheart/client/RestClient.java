...
public class RestClient {
    ...
    public static <T, E> E synchronousCall(String uri, T request, Class<E> bodyType) {
        ...
        E response = WebClient.create().post().uri(URI.create(uri)).body(BodyInserters.fromObject(request)).retrieve().bodyToMono(bodyType) //The method bodyToMono() extracts the body to a Mono instance
    ...
    public static <T, E> void asynchronousCall(String uri, T request, Class<E> bodyType) {
        ...
        WebClient.create().post().uri(URI.create(uri)).body(BodyInserters.fromObject(request)).retrieve().bodyToMono(bodyType).subscribe(); //Mono.subscribe() returns immediately (as opposed to block() method which waits for the full response)
